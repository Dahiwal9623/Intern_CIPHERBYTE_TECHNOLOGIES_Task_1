package Intern_C_T_Tk;

import java.util.*;

public class task_1_internship {

	Scanner scobj = new Scanner(System.in);
	HashMap<String, Integer> Details = new HashMap<String, Integer>();

	public void User_Login() {
		Details.put("Rohan", 3501);
		System.out.println("\n->>>> Lata Aabasaheb Kakde Collega <<<<-");
	...
	public void Examination() {
		...
		while (System.currentTimeMillis() < EndTime || sno < x) {
			String i = al.get(sno);
			System.out.println("\n\nQ." + (sno + 1) + ") " + i);
			System.out.println("\n(1) " + queries.get(i).get(0) + "\t" + "(2) " + queries.get(i).get(1) + "\t" + "(3) "
					+ queries.get(i).get(2) + "\t" + "(4) " + queries.get(i).get(3) + " ");
	...
	public static void main(String[] args) {
		task_1_internship em = new task_1_internship();
		em.User_Login();
	}
